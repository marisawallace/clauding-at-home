...
def resolve_archive_dir() -> Path:
    ...
    sources = parse_codex_sources(config)
    if not sources:
        raise RuntimeError(
            f"{CODEX_SOURCES_ENV_KEY} is not set in {ENV_FILE}. "
            f"Run `python migrations/004_setup_codex_archival.py` to configure."
        )
    ...
    for entry_host, path in sources:
        if entry_host == host:
            return path
    raise RuntimeError(
        f"No entry for host {host!r} in {CODEX_SOURCES_ENV_KEY}. "
        f"Run `python migrations/004_setup_codex_archival.py` on this machine."
    )
